...
export default function JobCard({ job, isFavorite = false }: JobCardProps) {
  ...
  return (
    <div className="relative @container/card group bg-white rounded-xl p-8 shadow-sm transition-all duration-500 hover:shadow-xl hover:-translate-y-1.5 flex flex-col h-full border border-zinc-50">
        ...
        <div className="relative size-16 shrink-0">
          {logo ? (
            <Image
              ...
              fill
              className="rounded-xl object-cover border border-zinc-100 p-2 shadow-inner"
              unoptimized
              loading="lazy"
            />
          ) : (
      ...
      <div className="flex-1 flex flex-col">
        <p className="text-zinc-600 font-medium text-sm mb-2">{companyName}</p>
        <h3 className="text-2xl font-display font-semibold text-primary uppercase leading-none mb-6 tracking-tight group-hover:text-primary-container transition-colors duration-300">
