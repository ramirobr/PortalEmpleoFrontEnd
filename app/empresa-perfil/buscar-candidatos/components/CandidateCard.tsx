    ...
    <div className="@container/card group bg-white rounded-xl p-8 shadow-sm transition-all duration-500 hover:shadow-xl hover:-translate-y-1.5 flex flex-col h-full border border-zinc-50">
        ...
        {topSkills.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-8">
            {topSkills.map((skill) => (
              <span
                key={skill}
                className="px-2.5 py-1 bg-zinc-50 text-slate-500 text-[11px] font-bold rounded-lg border border-zinc-100"
              >
                {skill}
              </span>
            ))}
      ...
      <div className="mt-auto">
        <PremiumButton
          className="w-full"
          onClick={() =>
            push(`/empresa-perfil/candidato/${candidate.idUsuario}`)
          }
        >
          Ver Perfil
        </PremiumButton>
