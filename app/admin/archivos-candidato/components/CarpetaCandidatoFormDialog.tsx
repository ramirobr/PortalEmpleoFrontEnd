...
import { CarpetaUsuario, CarpetaFormValues } from "@/types/admin";
...
interface CarpetaCandidatoFormDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  carpeta: CarpetaUsuario | null;
  carpetas: CarpetaUsuario[];
  onSubmit: (data: CarpetaFormValues) => Promise<void>;
  isSubmitting: boolean;
}

export default function CarpetaCandidatoFormDialog({
  open,
  onOpenChange,
  carpeta,
  carpetas,
  onSubmit,
  isSubmitting,
}: CarpetaCandidatoFormDialogProps) {
  const isEditing = !!carpeta;

  const form = useForm<CarpetaFormValues>({
    resolver: zodResolver(carpetaSchema) as Resolver<CarpetaFormValues>,
  ...
  const possibleParents = carpetas.filter(
    (c) => c.idCarpetaUsuario !== carpeta?.idCarpetaUsuario,
  );
        ...
        <DialogHeader>
          ...
          <DialogDescription>
            {isEditing
              ...
              : "Crea una nueva carpeta para organizar los archivos del candidato."}
          </DialogDescription>
        </DialogHeader>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Ej: CV, Certificados..." {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
