...
export default function AdminArchivosEmpresaPage() {
  ...
  const { carpetas, archivos, selectedCarpetaId, loadingEmpresas, loadingCarpetas, loadingArchivos } = dataState;
  ...
  const setArchivos = (val: ArchivoEmpresa[] | ((prev: ArchivoEmpresa[]) => ArchivoEmpresa[])) => {
    setDataState(prev => ({
      ...prev,
  ...
  const setLoadingEmpresas = (val: boolean | ((prev: boolean) => boolean)) => {
    setDataState(prev => ({
      ...prev,
      loadingEmpresas: typeof val === 'function' ? (val as Function)(prev.loadingEmpresas) : val
    }));
  };
