...
interface AdminCandidateIdiomasProps {
  idiomas: Idioma[];
}
...
export default function AdminCandidateIdiomas({ idiomas }: AdminCandidateIdiomasProps) {
  if (!idiomas.length) {
    return (
          ...
          return (
            <div key={idioma.id}>
              <div className="flex justify-between items-center mb-1">
                <span className="text-sm font-medium text-slate-900">{idioma.nombre}</span>
                <span className="text-xs text-slate-500">{idioma.nivel}</span>
              </div>
