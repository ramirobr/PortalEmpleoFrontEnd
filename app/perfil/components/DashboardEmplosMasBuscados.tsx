...
export default function DashboardEmplosMasBuscados() {
  return (
    <section aria-labelledby="popular-jobs-title">
      <h2 id="popular-jobs-title" className="text-lg font-semibold text-slate-800 mb-4">
          ...
          {EMPLEOS_POPULARES.map((empleo) => (
            <li key={empleo}>
              ...
              >
                ...
                <span className="text-sm text-slate-700 group-hover:text-primary flex-1 transition-colors">
                  {empleo}
                </span>
                <ChevronRight className="size-4 text-slate-400 group-hover:text-primary transition-colors" aria-hidden="true" />
              </Link>
            </li>
          ))}
        </ul>
