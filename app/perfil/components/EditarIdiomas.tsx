        ...
        {idiomas?.length ? (
          idiomas.map((item) => (
            ...
            >
              ...
              <button
                id="borrar-idioma"
                className="cursor-pointer text-slate-400 hover:text-slate-600 transition-colors mt-1"
      ...
      <Dialog open={!!pendingDeleteId} onOpenChange={cancelDelete}>
        <DialogContent className="p-8 max-w-md flex flex-col items-center">
          ...
          <div className="flex gap-4 mt-4">
            <PremiumButton variant="outline" onClick={cancelDelete}>
              Cancelar
            </PremiumButton>
            <PremiumButton onClick={confirmDelete}>Aceptar</PremiumButton>
          </div>
        </DialogContent>
      </Dialog>
    </Card>
