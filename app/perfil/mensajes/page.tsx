...
export default function MensajesCandidatoPage() {
  ...
  useEffect(() => {
    if (!MESSAGING_ENABLED) router.replace("/perfil");
  }, [router]);
  ...
  useEffect(() => {
    ...
    fetchConversacionesByUsuario(userId, session.user.accessToken).then((res) => {
      if (res?.isSuccess) {
  ...
  const handleSelectConv = useCallback(
    async (conv: Conversacion) => {
      ...
      await marcarLeidosByConversacion(conv.idConversacion, false, session.user.accessToken);
      setConversaciones((prev) =>
        prev.map((c) =>
  ...
  const handleSend = async (contenido: string) => {
    ...
    const res = await enviarMensaje(
      { idConversacion: selectedConv.idConversacion, esDeEmpresa: false, contenido },
          ...
          {selectedConv ? (
            <>
              <div className="px-5 py-3 border-b border-zinc-100">
                <p className="font-semibold text-slate-800">{selectedConv.nombreEmpresa}</p>
                {selectedConv.tituloVacante && (
                  <p className="text-xs text-primary/70">{selectedConv.tituloVacante}</p>
                )}
              </div>
              <ChatWindow mensajes={mensajes} esEmpresa={false} />
