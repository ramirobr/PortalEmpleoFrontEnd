...
export async function addVacanteFavorita(
  ...
  if (!json?.isSuccess) {
    console.error("Error guardando vacante como favorita:", json);
  }
}

export async function removeVacanteFavorita(
  idVacante: string,
  userId: string,
  token: string,
) {
...
export async function getVacantesFavoritasByUser(user?: User) {
  if (!user) return;
  const json = await fetchApi<GetLastEightJobsResponse>(
    `/Jobs/getVacantesFavoritasByUser/${user.id}`,
    {
